...
function delete_igw() {
    ...
    --region "${region}" | jq '.InternetGateways | .[] | .InternetGatewayId' | tr -d '"')
	if [ -n "$igws" ]; then
		for igw_id in $igws; do
			if aws ec2 detach-internet-gateway \
          --internet-gateway-id "${igw_id}" \
          --vpc-id "${vpc}" --region "${region}"; then

				aws ec2 delete-internet-gateway \
          --internet-gateway-id "${igw_id}" --region "${region}"
				echo "detached ${igw_id} from ${vpc} and deleted"
			fi
		done
	fi
...
function delete_subnets() {
    ...
    --region "${region}" | jq '.[] | .[].SubnetId' | tr -d '"')
...
function delete_route_tables() {
    ...
    --filters Name=vpc-id,Values="${vpc}" \
    --region "${region}" \
    | jq '.RouteTables | .[].Associations | .[] | select(.Main != true) | .RouteTableId' | tr -d '"')
	for rt_id in $rts; do
...
function delete_acls() {
    ...
    --filters Name=vpc-id,Values="${vpc}" \
    --region "${region}" \
    | jq '.NetworkAcls | .[] | select(.IsDefault != true) | .NetworkAclId' | tr -d '"')
...
function delete_security_groups() {
	local region=$1
	local vpc=$2
	sgps=$(aws ec2 describe-security-groups \
    --filters Name=vpc-id,Values="${vpc}" \
    --region "${region}" \
    | jq '.SecurityGroups | .[] | select(.GroupName != "default") | .GroupId' | tr -d '"')
...
function delete_dhcp_options() {
	local region=$1
	dhcp_ops=$(aws ec2 describe-dhcp-options \
    --filters "Name=key,Values=domain-name-servers,Name=value,Values=AmazonProvidedDNS" \
    --region "${region}" | jq '.DhcpOptions | .[] | .DhcpOptionsId' | tr -d '"')
